...
class Comment extends Base_Controller {
   ...
   if ($this->form_validation->run("api/v1/commentList") == true) {
	        ...
	        $contentData = $this->Common_model->selectData('wrng_comment','*', array('contentId' => $postDataArr['contentId'],'type' => $postDataArr['type']), $or_where = false, $ord = 'commentId', $ordtype = 'DESC', $grpby = false, $whstr_in = false, $lmt = $limit); 
