...
class OttContent extends CI_Controller {
	...
	foreach ($postDataArr['genre'] as $valueGenre) {
		...
		if(isset($_POST['videoSubmit'])){
			$columnArrr=$this->input->post();
						...
						if(!empty($_FILES['video_mp4']['name'])){
							...
							$columnArrr["video_mp4"] = 'http://localhost/WrangaWeb/wrangademo/assets/videoMP4/'.$pre.$file;
							$columnArrr["video_url"] = 'http://localhost/WrangaWeb/wrangademo/assets/videoMP4/'.$pre.$file;
		...
		$data['wrng_genre'] = $this->Common->selectData('wrng_genre', $fld = '*', ['categoryId'=>1], $or_where = false, $ord = 'name', $ordtype = 'ASC', $grpby = false, $whstr_in = false, $lmt = false,false);
	...
	if(empty($wrng_content_video_url)){
		...
		$data['ottType'] = $this->Common->selectData('ottType');
		$data['wrng_contentType'] = $this->Common->selectData('wrng_contentType');
		$data['wrng_genre'] = $this->Common->selectData('wrng_genre', $fld = '*', ['categoryId'=>2], $or_where = false, $ord = 'name', $ordtype = 'ASC', $grpby = false, $whstr_in = false, $lmt = false,false);
	...
	foreach ($postDataArr['genre'] as $valueGenre) {
		...
		$data['ottType'] = $this->Common->selectData('ottType');
		$data['wrng_contentType'] = $this->Common->selectData('wrng_contentType');
		$data['wrng_genre'] = $this->Common->selectData('wrng_genre', $fld = '*',['categoryId'=>3], $or_where = false, $ord = 'name', $ordtype = 'ASC', $grpby = false, $whstr_in = false, $lmt = false,false);
	...
	public function updateContentPublishStatus($contentId){
		...
		if($conutTotalValue && $genralInfoValidation && $wrng_shortreview && $wrng_tipsList){
	...
	public function genre(){
		...
		$data['wrng_genre'] = $this->Common->selectData('wrng_genre', $fld = '*', $conditions = false, $or_where = false, $ord = 'gId', $ordtype = 'DESC', $grpby = false, $whstr_in = false, $lmt = false,false);
		$data['wrng_category'] = $this->Common->selectData('catagories');
...
$data['sexismRating'] =$this->Common->selectData('moralBoard', $fld = '*',['contentId'=>$id,'reatingCalCategoryId'=>10]);
	...
	if($postDataArr){	
		...
		$this->load->view('short-review',$data);
	...
	public function tipsandTricks(){
		...
		$data['content'] =$this->Common->selectData('content', $fld = '*',['contentId'=>$this->uri->segment(2)]);
		$data['wrng_tipslistss'] =$this->Common->selectData('wrng_tipsList', $fld = '*',['contentId'=>$this->uri->segment(2)]);
		$data['platform'] =$this->Common->selectData('platform');
		$data['content'] = $this->Common->selectData('content','*',['contentId'=>$this->uri->segment(2)]);

		$this->load->view('tips-and-tricks',$data);
	}
	...
	public function paymentPlan(){
		...
		if(isset($postDataArr['Create'])){
			unset($postDataArr['Create']);
				...
				$postDataArr["status"] =1;
				$id=$this->Common->insert('wrng_paymentPlan',$postDataArr);	
				if($id){
					header("Location:http://localhost/WrangaWeb/wrangademo/index.php/paymentPlan");					
				}
			}
		...
		$data['reviewerCount'] = $this->Common->nrows('cms_admin','*',['user_type'=>4]);
		$data['mobileCount'] = $this->Common->nrows('users');
		$data['paymentPlan']=$this->Common->selectData('wrng_paymentPlan','*',array("status"=>1), $or_where = false, $ord = false, $ordtype = false, $grpby = false, $whstr_in = false, $lmt = false,$where_not_in= false);
		$this->load->view('paymentPlan',$data);
	}
	...
	public function planBenifits(){
		$data=array();
		$postDataArr=$this->input->post();
		if(isset($postDataArr['submitBenifits'])){
			unset($postDataArr['submitBenifits']);
			$id=$this->Common->insert('wrng_planBenifits',$postDataArr);	
				if($id){
					header("Location:http://localhost/WrangaWeb/wrangademo/index.php/planBenifits");					
				}
			}		
		$data['adminCount'] = $this->Common->nrows('cms_admin','*',['user_type'=>2]);
		$data['teamLeaderCount'] = $this->Common->nrows('cms_admin','*',['user_type'=>3]);
		$data['reviewerCount'] = $this->Common->nrows('cms_admin','*',['user_type'=>4]);
		$data['mobileCount'] = $this->Common->nrows('users');
		$data['wrng_planBenifits']=$this->Common->selectData('wrng_planBenifits','*');
		$data['wrng_paymentPlan']=$this->Common->selectData('wrng_paymentPlan','*');
		$this->load->view('planBenifits',$data);
	}
	...
	public function updatePassword($id=NULL){
			...
			$postDataArr=$this->input->post();
				...
				if($postDataArr){	
					/*if($postDataArr['pass']==$postDataArr['cpass']){*/
						$postDataArrr['password']=$postDataArr['pass'];
						$data=$this->Common->update('cms_admin',$postDataArrr,['id'=>$id]);
							header("location:". base_url_index);
					/*}else{
						echo "<script>alert('Your Pawword did not Match');SONU<script>";
					}*/
				}	
	...
	public function forgetPassword(){
		$data=array();
		$postDataArr=$this->input->post();
		//$postDataArr['email']='kaushlendras1@gmail.com';
		
		
		if($postDataArr){
					$data=$this->Common->selectData('cms_admin','*',['email'=>$postDataArr['email']]);
					$this->Common->update_single('users',$postDataArr,['email'=>$postDataArr['email']]);
					$to=$postDataArr['email'];
					$subject="Wranga:ForgetPassword";
					$id=1;
					 $message = "
                          <html><body>
                          ...
                          <table>
                          <tr>
                          <th><a href='http://localhost/WrangaWeb/wrangademo/index.php/updatePassword/".$id."'>Click Here</a></th>
                          </tr>
                          </table>
                          </body>
                          </html>"; 
					...
					$this->email->from('admin@wranga.in', 'Support name'); // change it to yours
